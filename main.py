...
class AudioClassifier:
    ...
    def inference(self, request: InferenceRequest):
        ...
        with torch.no_grad():
            ...
            top3_probs, top_indices = torch.topk(probabilities[0], 3)

            predictions = [{"class": self.classes[idx.item()], "confidence": prob.item(
            )} for prob, idx in zip(top3_probs, top_indices)]
...
def main():
    audio_data, sample_rate = sf.read("drums.wav")
