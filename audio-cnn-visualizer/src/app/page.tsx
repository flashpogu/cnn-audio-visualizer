...
export default function HomePage() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      const response = await fetch("https://zeddx-rahul--audio-cnn-inference-audioclassifier-inference.modal.run/", {
        method: "POST",
    ...
    <main className="min-h-screen bg-stone-50 p-8">
        <div className="mx-auto max-w-[90%]">
          <div className="mb-12 text-center">
            <h1 className="mb-4 text-4xl font-light tracking-tight text-stone-900">CNN Audio Visualiser</h1>
            <p className="text-md mb-8 text-stone-600">
              Upload a WAV file to see the model&apos;s predictions and feature map
            </p>
            <div className="flex flex-col items-center">
              <div className="relative inline-block">
                <input type="file" accept=".wav" id="file-uplaod"
                onChange={handleFileChange}
                disabled={isLoading}
                 className="absolute inset-0 w-full cursor-pointer opacity-0" />
                <Button disabled={isLoading} className="border-stone-300" variant="outline" size="lg">{isLoading ? "Analyzing.." : "Choose File"}</Button>
              </div>
              {fileName && (<Badge variant="secondary" className="mt-4 bg-stone-200 text-stone-700">{fileName}</Badge>)}
            </div>
          </div>

          {error && (<Card className="mb-8 border-red-200 bg-red-50"><CardContent><p className="text-red-600">
            Error: {error}</p></CardContent></Card>)}

            {vizData && (<div className="space-y-8">
              <Card>
                <CardHeader>
                  <CardTitle>Top Predictions</CardTitle></CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Convulational Layer Output</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-5 gap-6">
                    {main.map(([mainName, mainData]) => (
                      <div key={mainName} className="space-y-4">
                        ...
                        {internals[mainName] && (
                          <div className="h-80 overflow-y-auto rounded-border border-stone-200 bg-stone-50">
